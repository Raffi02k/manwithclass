...
function structuredData(path) {
    ...
    const business = {
        '@type': 'HairSalon',
        '@id': site_1.site.domain + '/#salon',
        name: site_1.site.fullName,
        url: site_1.site.domain + '/',
        image: site_1.site.domain + '/images/salon-wide.webp',
        logo: site_1.site.domain + '/images/logo.png',
        telephone: '+46709627503',
        email: site_1.site.email,
        priceRange: '$$',
        address: {
            '@type': 'PostalAddress',
            streetAddress: site_1.site.address,
            postalCode: site_1.site.postcode,
            addressLocality: site_1.site.city,
        ...
        hasMap: site_1.site.mapsUrl,
        sameAs: [site_1.site.instagram, site_1.site.facebook, site_1.site.bookingUrl],
    ...
    const graph = [
        business,
        { '@type': 'WebSite', '@id': site_1.site.domain + '/#website', url: site_1.site.domain + '/', name: site_1.site.fullName, inLanguage: ['sv', 'en'] },
        { '@type': 'WebPage', '@id': meta.canonical + '#webpage', url: meta.canonical, name: meta.title, description: meta.description, inLanguage: meta.lang, about: { '@id': site_1.site.domain + '/#salon' }, isPartOf: { '@id': site_1.site.domain + '/#website' } }
    ];
    if (meta.service) {
        ...
        graph.push({
            ...
            provider: { '@id': site_1.site.domain + '/#salon' },
            areaServed: [
                { '@type': 'AdministrativeArea', name: 'Odenplan' },
                { '@type': 'AdministrativeArea', name: 'Vasastan' },
                { '@type': 'City', name: 'Stockholm' }
            ],
            ...(s.from ? {} : { offers: { '@type': 'Offer', price: s.price, priceCurrency: 'SEK', availability: 'https://schema.org/InStock', url: site_1.site.bookingUrl } })
        });
        graph.push({
            '@type': 'BreadcrumbList',
            ...
            itemListElement: [
                {
                    ...
                    item: site_1.site.domain + (meta.lang === 'sv' ? '/' : '/en')
                },
                {
                    '@type': 'ListItem',
                    position: 2,
                    name: meta.lang === 'sv' ? 'Tjänster & priser' : 'Services & prices',
                    item: site_1.site.domain + (meta.lang === 'sv' ? '/tjanster' : '/en/services')
                },
                {
                    '@type': 'ListItem',
                    position: 3,
